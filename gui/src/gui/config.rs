...
#[builder_struct_attr(serde(default))]
pub struct Config {
    pub cell_size: i8,
    pub text_size: f32,
    ...
    #[builder_field_attr(serde(
        deserialize_with = "option_deserializer::<_, KeyboardShortcut, egui::KeyboardShortcut>"
    ))]
    ...
    #[builder_field_attr(serde(
        deserialize_with = "option_deserializer::<_, TranslationCtx, JSONGetText<'static>>"
    ))]
    pub translation_ctx: JSONGetText<'static>,
    pub initial_language: String,
    #[builder_field_attr(serde(
        deserialize_with = "option_deserializer::<_, LevelFilter, log::LevelFilter>"
    ))]
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            translation_ctx: static_json_gettext_build!(
                "en_UK";
...
impl PartialEq for Config {
    fn eq(&self, other: &Self) -> bool {
        self.cell_size == other.cell_size
            ...
            && self.translation_ctx.get_default_key() == other.translation_ctx.get_default_key()
    ...
    fn validate(&self) -> Result<(), ConfigBuilderError> {
            ...
            .is_some_and(|ctx| !ctx.contains_key(initial_language))
        {
            return Err(ConfigBuilderError::ValidationError(format!(
                ...
                self.translation_ctx.as_ref().unwrap().get_keys()
            )));
        }
        ...
        let modifiers = self
            ...
            .map(|modifier| -> Result<egui::Modifiers, Self::Error> {
                match modifier {
                    ...
                    name => Err(de::Error::invalid_value(
                        Unexpected::Str(name.into_boxed_str().as_ref()),
...
impl TranslationCtx {
    fn map_error(error: json_gettext::JSONGetTextBuildError) -> de::value::Error {
        use json_gettext::{JSONGetTextBuildError, Key};
        match error {
            JSONGetTextBuildError::DefaultKeyNotFound => de::Error::missing_field("default_key"),
            JSONGetTextBuildError::TextInKeyNotInDefaultKey { key, text } => {
                de::Error::custom(format!("text \"{text}\" not found in \"{key}\" "))
            }
            JSONGetTextBuildError::DuplicatedKey(Key(key)) => {
                de::Error::custom(format!("translation \"{key}\" is already defined"))
            }
            JSONGetTextBuildError::IOError(error) => de::Error::custom(format!("{error}")),
            JSONGetTextBuildError::SerdeJSONError(error) => de::Error::custom(format!("{error}")),
        }
    }
}
...
mod config_tests {
    ...
    fn test_validation_initial_language() {
        ...
        let initial_language = "sk_SK".to_owned();
        assert!(ConfigBuilder::create_empty()
            .initial_language(initial_language.clone())
            .translation_ctx(ctx_builder.build().unwrap())
            .build()
            .is_err_and(|err| {
                match err {
                    ConfigBuilderError::UninitializedField(_) => false,
    ...
    fn test_deserialize_full_config() {
        ...
        let config_builder: ConfigBuilder = serde_json::from_str(stringify!({
            ...
            "translation_ctx": { "default_key": "en_UK", "translations": [{"key": "en_UK", "path": "locales/en_UK.json"}] },
